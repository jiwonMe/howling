...
이 페이지에서 HA URL·long-lived token을 저장하고 Pairing 시작을 누릅니다. 토큰은 `data/secrets/ha-token`에 mode `0600`으로 씁니다. 클라우드 Connections 화면은 metadata와 code 입력만 있습니다. secret 재조회 API는 없습니다.
